...
% Last Modified by GUIDE v2.5 24-Jan-2017 12:00:01
...
% Log Sweep creation
[sweep,invSweep,R] = logSweep(N,f1,f2,[handles.lossL handles.lossH],fs);
handles.sweep = sweep;
...
%     handles.N = N; 
    ...
    handles.lossL = lossL; 
    handles.lossH = lossH;
    [handles.sweep,handles.invSweep,handles.R] = logSweep(handles.N,handles.f1,handles.f2,[lossL lossH],fs);
...
audiowrite('invSweep.wav',handles.invSweep,fs,'BitsPerSample',32);
else
    wavwrite(handles.sweep,fs,'sweep');
    wavwrite(handles.invSweep,fs,'invSweep');
end;
guidata(hObject, handles);
